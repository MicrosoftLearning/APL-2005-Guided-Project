...
{
    private const string FilePath = "Plugins/flights.json";
    ...
    [Description("Searches for available flights based on the destination")]
    [return: Description("A list of available flights")]
    public List<FlightModel> SearchFlights(string destination)
    {
        // Filter flights based on destination
        return flights.Where(flight =>
            flight.Destination.Equals(destination, StringComparison.OrdinalIgnoreCase)).ToList();
    }
    ...
    public string BookFlight(int flightId)
    {
        ...
        if (flight.IsBooked)
        {
            return $"Flight ID {flightId} is already booked.";
        }
